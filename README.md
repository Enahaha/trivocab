...
- 学习方式可选：简易模式（当前方式）与强化模式（不背单词式：选项首学 + 形近干扰项、组内多次回忆、组末拼写），设置页随时切换
...
- `TrVocab-1.2.0.exe`：安装器（需要 Java 21+ 或由 CI 内置运行时，推荐便携版）
- `TrVocab-1.2.0-windows.zip`：便携版，解压后运行 `TrVocab\TrVocab.exe` 即可
...
- `AGAIN`：本组延后 3 个卡片再出现；若中途退出，10 分钟后进入待复习队列，并重置长期间隔计数。
- `HARD`：本组延后 3 个卡片再出现；若中途退出，30 分钟后进入待复习队列，不推进长期间隔。
- `GOOD`：结束该词本组循环，按 SM-2 风格安排后续复习：首次成功 1 天后，之后间隔 = `round(上次间隔 × ease)`，上限 365 天。
- `EASY`：立即标记为 `MASTERED`，首次 30 天后回到复习队列；之后答 `GOOD` / `EASY` 会按 `round(上次间隔 × ease)` 继续延长间隔。
- 已掌握（`MASTERED`）的词到期后自动回到复习队列：答 `GOOD` / `EASY` 保持 `MASTERED` 并延长间隔，答 `AGAIN` / `HARD` 自动降级回 `NEW` / `LEARNING`，避免遗忘后无补救路径。
- `ease` 初始 2.5，范围 1.3～3.0：`AGAIN` -0.20、`HARD` -0.10、`EASY` +0.15、`GOOD` 不变。
...
每日计划支持 `10、20、30……100` 个单词。预计天数按“剩余尚未首次学习的单词数 ÷ 每日目标”向上取整，预计完成日期以**用户所在时区**的当地日期为起点；该估算表示完成词书的首轮学习，不包含之后会随学习反馈变化的长期复习时间。保存后，仪表盘和学习队列都会使用当前用户的计划。
...
- 按照当前需求，没有实现单词或例句朗读。
- 组内“延后 3 张卡片”的重排由前端在浏览器本地持久化（localStorage 快照），刷新或重新进入页面可继续未完成的学习会话；跨设备或清除浏览器数据后不会恢复。
- 签到、每日统计与连续天数按各账号保存的个人时区计算（注册 / 登录时由浏览器自动上报，默认 `Asia/Seoul`）。
